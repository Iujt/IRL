...
function renderTeams(teams, drivers, logoById, flagById) {
  ...
  container.innerHTML = teams
    .map((team) => {
      const logo = (logoById[team.logoId]?.image || logoById[team.logoId]?.logo) || "";
      const teamDrivers = driversByTeam[team.id] || [];
      const rows =
              ...
              .map((driver) => {
                ...
                return `
                  <li class="lineup-driver">
                    ${
                      flag
                        ? `<img class="driver-flag driver-flag--lg" src="${flag}" alt="${driver.country || "Driver"} flag" />`
                        : ""
                    }
        ...
        <article class="card lineup-card">
          <div class="lineup-team-head">
            <div class="lineup-team-id">
              ${
                logo
                  ? `<img class="team-logo team-logo--lg" src="${logo}" alt="${team.name} logo" />`
                  : ""
              }
              <h3>${team.name}</h3>
            </div>
