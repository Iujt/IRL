        ...
        <div class="steward-grid">
          ${(entry.decisions || [])
            ...
            <div class="race-card decision-card">
              <div>
                <strong>${escapeHTML(decision.driver || "Unknown Driver")}</strong>
                <div class="race-meta">${escapeHTML(decision.incident || "—")}</div>
                <div class="race-meta">${escapeHTML(decision.notes || "No additional notes.")}</div>
              </div>
              <div class="badge same">${escapeHTML(decision.penalty || "No penalty")}</div>
            </div>
          `
            )
            .join("")}
...
(async function init() {
  try {
    ...
    if (list) {
      list.innerHTML = `<p class="race-meta">Failed to load stewarding data: ${escapeHTML(error?.message || error)}</p>`;
    }
  }
})();
