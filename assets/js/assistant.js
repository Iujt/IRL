...
(async function init() {
  ...
  if (aiStatusEl) {
    if (!AI_ENDPOINT) {
      aiStatusEl.textContent = "AI unavailable (site.json has no aiEndpoint).";
    } else {
      let host = AI_ENDPOINT;
      try {
        host = new URL(AI_ENDPOINT).host;
      } catch {}
      aiStatusEl.textContent = `AI ready (${host}).`;
    }
  }
