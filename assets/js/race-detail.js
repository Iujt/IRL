...
import { computeRacePoints, formatResultPosition } from "./standings-calc.js";
  ...
  function renderSession(sessionKey) {
    ...
    tbody.innerHTML = session.data.results
      .map((result) => {
        ...
        const teamName = team?.name || result.teamId || "Unknown Team";
        const penalties = Array.isArray(result.penalties) ? result.penalties : [];
        return `
        <tr>
          <td>${formatResultPosition(result)}</td>
          <td>
          ...
          <td>${penalties.length ? penalties.join(", ") : "—"}</td>
          <td>${result.status}</td>
        </tr>
      `;
      })
