...
function resolveLogo(logoId, logosById) {
  ...
  const raw = logosById[logoId];
  const value =
    typeof raw === "string" ? raw : (raw && (raw.image || raw.logo)) ? (raw.image || raw.logo) : "";
  if (value) {
    // If someone stores just "haas.png" (or similar), make it usable.
    if (!value.includes("/") && !value.startsWith("http")) return `assets/img/teams/${value}`;
    return value;
  }
  return `assets/img/teams/${logoId}.png`;
}
