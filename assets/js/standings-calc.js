...
export function computeRacePoints(result, pointsConfig, sessionType) {
  ...
  const manualBasePoints = Number(result?.pointsOverride);
  // A points override replaces only position points. Fastest-lap points remain
  // separate, so shared positions and steward adjustments retain normal FL rules.
  const basePoints = Number.isFinite(manualBasePoints)
    ? manualBasePoints
    : pointsTable?.[result.position - 1] || 0;
...
export function formatResultPosition(result, includePrefix = false) {
  const position = result?.sharedPosition ?? result?.position;
  if (position == null || position === "") return "—";
  const label = `${includePrefix ? "P" : ""}${position}`;
  return result?.sharedPosition != null ? `${label} (tie)` : label;
}
