...
import { computeStandingsWithChange, computeRacePoints, formatResultPosition } from "./standings-calc.js";
...
const nextRaceNameEl = document.querySelector("#next-race-name");
const nextRaceDateEl = document.querySelector("#next-race-date");
const standingsPreviewEl = document.querySelector("#standings-preview");
const latestRaceEl = document.querySelector("#latest-race");
...
function startCountdown(targetDate) {
  if (!countdownSlots.days || !countdownSlots.hours || !countdownSlots.minutes || !countdownSlots.seconds) {
    return;
  }
...
function renderStandingsPreview(standings, driversById, teamsById, flagsById, logosById) {
  if (!standingsPreviewEl) return;
  const top = standings.driverStandings.slice(0, 5);
  standingsPreviewEl.innerHTML = top
    .map((row) => {
      ...
      const driverName = driver?.name || row.driverId || "Unknown Driver";
      const driverCountry = driver?.country || "";
      const teamName = team?.name || row.teamId || "Unknown Team";
      return `
      <div class="race-card">
        <div>
          <strong>#${row.position} ${driverName}</strong>
          <div class="race-meta">
            ${flag ? `<img class="driver-flag driver-flag--lg" src="${flag}" alt="${driverCountry} flag" />` : ""}
            ${logo ? `<img class="team-logo team-logo--lg" src="${logo}" alt="${teamName} logo" />` : ""}
            ${teamName}
          </div>
        </div>
    ...
    <div class="timeline">
      ${topThree
        .map((result) => {
          ...
          const flag = resolveFlag(driver?.flagId, flagsById);
          const driverName = driver?.name || result.driverId || "Unknown Driver";
          const driverCountry = driver?.country || "";
          return `
        <div class="race-card">
          <div>
            <strong>${formatResultPosition(result, true)} ${driverName}</strong>
            <div class="race-meta">${flag ? `<img class="driver-flag" src="${flag}" alt="${driverCountry} flag" />` : ""} ${result.time}</div>
          </div>
...
(async function init() {
  ...
  if (upcoming) {
    ...
    if (nextRaceNameEl) nextRaceNameEl.textContent = upcoming.name;
    if (nextRaceDateEl) nextRaceDateEl.textContent = `${formatDate(upcoming.date)} • ${track}`;
